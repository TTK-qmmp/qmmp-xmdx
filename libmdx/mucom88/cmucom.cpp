...
void CMucom::Init(int option, int rate)
{
  ...
  if (rate == 0)
    AudioCurrentRate = MUCOM_AUDIO_RATE;
...
int CMucom::Play(int num, bool start)
{
  ...
  if (hedmusic->pcmdata)
  {
    ...
    {
      // 既に同名のPCMが読み込まれているか?
      if (strcmp(pcmname, pcmfilename) == 0)
        skippcm = 1;
    }
    if (skippcm==0)
    {
...
void CMucom::GetExtramVector()
{
  ...
  if (vm->Peek(eram_tbl) != 0xc3)
    return;
  if (vm->Peek(eram_tbl + 9) != 0xc3)
    return;
  use_extram         = true;
...
int CMucom::LoadPCM(const char* fname)
{
  ...
  // (戻り値が0以外の場合はエラー)
  //
  if (strcmp(pcmfilename, fname)==0)
    return 0;                                                           // 既に読み込んでいる場合はスキップ
  strncpy(pcmfilename, fname, MUCOM_FILE_MAXSTR-1);
  if (vm->LoadPcm(fname) == 0)
    return 0;
  return -1;
}
...
MUCOM88_VOICEFORMAT * CMucom::GetFMVoice(int no)
{
  ...
  // (no=音色番号0～255)
  //
  if ((no < 0) || (no >= MUCOM_FMVOICE_MAXNO))
    return NULL;
...
void CMucom::EditorSetFileName(const char* filename, const char* pathname, bool sessionstart)
{
  ...
  //
  if (filename)
    edit_filename = std::string(filename);
  if (pathname)
...
int CMucom::CheckEditorUpdate(void)
{
  ...
  if (edit_status == MUCOM_EDIT_STATUS_NONE)
    return MUCOM_EDIT_STATUS_NONE;
...
int CMucom::Compile(char* text, int option, bool writeMub, const char* filename)
{
  ...
  if (tcount[maxch - 1] == 0)
    pcmflag = 2;                                        // PCM chが使われてなければPCM埋め込みはスキップ
...
int CMucom::GetDriverModeString(const char* name)
{
  ...
  if (*name == 0)
    res = MUCOM_DRIVER_NONE;
  if (STRCASECMP(name, "mucom88") == 0)
    res = MUCOM_DRIVER_MUCOM88;
  if (STRCASECMP(name, "mucom88e") == 0)
    res = MUCOM_DRIVER_MUCOM88E;
  if (STRCASECMP(name, "mucom88em") == 0)
    res = MUCOM_DRIVER_MUCOM88EM;
  if (STRCASECMP(name, "mucomdotnet") == 0)
    res = MUCOM_DRIVER_MUCOMDOTNET;
  return res;
}
...
int CMucom::MUBGetHeaderVersion(MUBHED* hed)
{
  char* p;
  p = (char *)hed;
  if ((p[0] == 'M') && (p[1] == 'U') && (p[2] == 'C') && (p[3] == '8'))
    return MUCOM_HEADER_VERSION1;
  if ((p[0] == 'M') && (p[1] == 'U') && (p[2] == 'B') && (p[3] == '8'))
    return MUCOM_HEADER_VERSION2;
  return -1;
}
