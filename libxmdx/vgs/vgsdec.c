...
int vgsdec_load_bgm_from_file(void* context, const char* path)
{
    ...
    fclose(fp);
    vgsdec_load_bgm_from_memory(context, data, size);
    free(data);
    return 0;
}
 ...
 *----------------------------------------------------------------------------
 */
void reset_context(struct _VGSCTX* c)
{
    int i;
    c->play = 1;
    c->mask = 0;
...
size_t extract_meta_data(struct _VGSCTX* c, void* data, size_t size)
{
    size_t msize = 0;
    char* d = (char*)data;
    int i;
...
void release_meta_data(struct _VGSCTX* c)
{
    int i;
    if (NULL != c->mhead) {
        if (NULL != c->mdata) {
...
void lock_context(struct _VGSCTX* c)
{
    pthread_mutex_lock(&(c->mt));
}

void unlock_context(struct _VGSCTX* c)
{
    pthread_mutex_unlock(&(c->mt));
}

void lock_queue(struct _VGSCTX* c)
{
    pthread_mutex_lock(&(c->queue.mt));
}

void unlock_queue(struct _VGSCTX* c)
{
    pthread_mutex_unlock(&(c->queue.mt));
}

/* set tone and tune */
void set_note(struct _VGSCTX* c, unsigned char cn, unsigned char t, unsigned char n)
{
    n += c->addKey[cn];
    if (c->ch[cn].toneM) {
        c->ch[cn].toneS = NULL;
...
/* get next waittime */
int get_next_note(struct _VGSCTX* c)
{
    int ret;
...
/* jump specific time */
void jump_time(struct _VGSCTX* c, int sec)
{
    int wt;
    reset_context(c);
    while (0 < sec) {
...
void* async_manager(void* context)
{
    struct _VGSCTX* c = (struct _VGSCTX*)context;
